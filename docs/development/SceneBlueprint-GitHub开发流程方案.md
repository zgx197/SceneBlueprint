...
### 6.4 自动 Release / 桌面打包发布（已具备最小手动链路）

当前已经补充了一条最小可用的手动桌面发布链路，用于 MVP 阶段打通：

- `release-desktop.yml` 支持 `workflow_dispatch`
- 当前优先支持 Windows 构建
- 当前会创建或更新基于版本号的 GitHub Release，并上传构建产物
...
- `release-desktop.yml`：MVP 阶段最小可用的手动 Windows 桌面发布流程

这意味着 SceneBlueprint 的第一套 GitHub 开发流程已经从“方案设计”进入“基础落地”阶段，并且已经具备最小可用的桌面发布闭环。
