...
**superseded_by**: [SceneBlueprint核心设计原则.md](../SceneBlueprint核心设计原则.md)（第八章）

> ⚠️ **归档说明（2026-02-19）**：本文档描述的数据流节点方案（`Location.RandomArea` / `Monster.Pool` / `Spawn.Execute`）已被**场景中心化设计**取代。  
> 相关节点已全部删除，运行时随机位置路径不再支持。当前唯一刷怪路径：`Spawn.Preset`（绑定 PointMarker）+ `Spawn.Wave`（绑定 AreaMarker）。  
> 本文档仅作历史参考，不得用于新功能设计依据。
...
**当前实现说明（2026-02-17）**：
- 类型兼容性由 `DataTypeRegistry` 统一托管，而非散落在各验证点。
- 支持数组元素类型递归检查（如 `Vector3[]` 仅兼容 `Vector3[]`）。
- 预留并支持基类型链的子类型检查（`derived -> base`）。
...
### 6.5 Scene 预览刷新机制（Location.RandomArea）

为保证数据流节点在 Scene 中的绿色预览“准且快”，当前编辑器实现已从轮询切到事件驱动：

1. **事件驱动脏标记**：节点属性变更、Marker 属性变更、节点增删等事件只入脏队列。  
2. **批次合并刷新**：通过 `delayCall` 合并同帧多次变化，统一执行 `FlushDirtyPreviews`。  
3. **Marker→Node 反向索引**：按 `markerId` 快速命中关联 `Location.RandomArea` 节点，减少全图扫描。  
4. **节点签名短路**：若节点参数 + Marker 几何签名未变化，跳过重复 `GenerateLocationPreview`。  
5. **一致性兜底**：索引空命中可重建，删除节点会清理 preview/signature 缓存，避免 stale 数据。
...
### Phase 5：预览刷新性能优化（1 天）✅ **已完成**

**目标**：提升 `Location.RandomArea` Scene 预览在复杂编辑场景下的性能与一致性

- [x] 轮询刷新改为事件驱动脏刷新（节点/全量双轨）
- [x] 按 MarkerId 精准定位节点刷新，接入 Marker 属性变更事件
- [x] 引入 `markerId -> nodeIds` 反向索引，减少全图扫描
- [x] 引入节点签名短路，签名未变化时跳过重复生成
- [x] 批量移除预览并合并重绘，降低 SceneView 重绘频率
...
- **`SceneBlueprint核心设计原则.md`**：🔴 必读 - 场景中心化、职责分离、单向数据流核心理念
...
**文档版本历史**：
- v1.1 (2026-02-17): 补充 DataTypeRegistry 统一类型兼容说明；新增 Scene 预览脏刷新优化（marker 索引 + 节点签名短路）；新增 Phase 5 记录
