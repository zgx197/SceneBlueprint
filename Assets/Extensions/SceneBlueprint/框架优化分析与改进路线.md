...
| 需求 | 当前可行性 | 主要阻碍 | 推荐方向 |
|------|-----------|----------|----------|
| **暂停** ✅ | 已实现（`BlueprintDebugController.IsPaused`） | 场景侧副作用不感知暂停 | `DebugController` 挂载到 Runner；IsPaused=true 时跳过 Tick |
...
| ✅ **BlueprintRunner 暂停 API** | SceneBlueprint.Runtime | `BlueprintDebugController.IsPaused`；Runner.Tick() 检测后跳过执行；完整 Pause/Resume/StepBack/StepForward/SeekToTick API |
...
| ✅ **BlueprintFrame 快照 API（只读检视）** | SceneBlueprint.Runtime | `BlueprintFrameSnapshot` + `BlueprintFrameHistory` 环形缓冲已实现；`TakeSnapshot/RestoreSnapshot`（完整回滚）仍需解决 Blackboard 装箱问题 |
