...
| ✅ **Blackboard 类型安全存储** | SceneBlueprint.Runtime | `_declared` 改为 `Dictionary<int, (object Value, Type Type)>`；`Set<T>` 写入时记录 `typeof(T)`；`GlobalBlackboard` 同步改造；快照层 `BlackboardEntry` 可直接从 `kvp.Value.Type.Name` 获取类型 |
...
**效果**：`Runtime.Systems` 程序集不再 `using SceneBlueprint.Actions.*`，端口字符串有编译时常量保护。
...
- `_declared` 改为 `Dictionary<int, (object Value, Type Type)>`
- `Set<T>` 存入时记录 `typeof(T)`；`Get<T>` / `TryGet<T>` 不变（仍走 `is T` pattern match）
- `DeclaredEntries` 暴露 `IReadOnlyDictionary<int, (object Value, Type Type)>`

消费方更新：
- `BlueprintFrameSnapshot.CaptureFrom`：`kvp.Value.Value` 取值，`kvp.Value.Type.Name` 兜底类型
