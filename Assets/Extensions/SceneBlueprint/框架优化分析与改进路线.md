...
- `PortDefinition.Required` 字段存在；**T4-2 的 SB002 规则**已在分析阶段强制检查未连接的 Required DataIn 端口（⚡ 编辑期提示已有，导出门控已通过 T4-4 实现）
- `ActionDefinition` 没有 `Validate(context)` 钩子——每种节点的自定义合法性检查仍只能通过新增 `IBlueprintRule` 来扩展，尚无 Definition 级注册机制
