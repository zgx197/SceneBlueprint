...
| **Phase 2** | 蓝图编辑器窗口（替代旧 SpawnBlueprintWindow） | Phase 1 | 编辑器中可创建/编辑蓝图 |
| **Phase 3** | 数据导出（BlueprintExporter → SceneBlueprintData） | Phase 1 | 导出 JSON 并反序列化验证 |
| **Phase 4** | 场景绑定（SceneBinding + SceneBlueprintManager） | Phase 2 + 3 | 场景对象绑定可往返序列化 |
...
单元测试：
  ✅ 编辑器窗口可打开不报错
  ✅ 从 ActionRegistry 创建节点到图中，节点数据正确
  ✅ Inspector 根据 ActionDefinition 生成控件（mock 验证）
  ✅ 节点间连线创建 TransitionEdgeData 正确

冒烟测试（编辑器窗口）：
  → 打开蓝图编辑器
  → 右键创建 Start 节点 + Spawn 节点 + Delay 节点
  → 连线：Start → Spawn → Delay
  → 选中 Spawn 节点，Inspector 显示属性面板，可编辑
  → 修改属性值，确认节点摘要文本更新
```
...
```
单元测试：
  ✅ SceneBlueprintData 全家族数据类可正确构造
  ✅ BlueprintExporter 将简单图（Start → Spawn → End）导出为正确的 JSON
  ✅ 导出的 JSON 反序列化后与原始数据一致
  ✅ 验证规则：缺少 Start 节点 → Error
  ✅ 验证规则：孤立节点 → Warning
  ✅ IAssetIdResolver 默认实现返回正确标识

冒烟测试（MenuItem）：
  → 加载一个测试蓝图资产
  → 调用 BlueprintExporter.Export()
  → 将 JSON 写入文件并在 Console 打印摘要（N 个行动，M 条过渡）
  → 打开导出的 JSON 文件人工确认结构正确
```
...
```
单元测试：
  ✅ SceneBindingEntry 序列化/反序列化正确
  ✅ Area 类型绑定导出顶点坐标数组
  ✅ Transform 类型绑定导出字符串 ID

冒烟测试（编辑器窗口）：
  → 在蓝图中创建一个 Spawn 节点
  → 在 Inspector 中选择场景中的一个区域对象作为 spawnArea
  → 导出 JSON，确认绑定数据正确
```
