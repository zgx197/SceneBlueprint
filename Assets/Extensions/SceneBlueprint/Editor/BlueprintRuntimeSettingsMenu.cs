...
namespace SceneBlueprint.Editor
{
    ...
    public static class BlueprintRuntimeSettingsMenu
    {
        ...
        private static Runtime.BlueprintRuntimeSettings? GetOrCreateSettings()
        {
            ...
            // 确保目录存在
            string directory = Path.GetDirectoryName(Runtime.BlueprintRuntimeSettings.AssetPath);
            ...
            AssetDatabase.CreateAsset(settings, Runtime.BlueprintRuntimeSettings.AssetPath);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            UnityEngine.Debug.Log($"[BlueprintRuntimeSettings] 配置文件已创建: {Runtime.BlueprintRuntimeSettings.AssetPath}");
